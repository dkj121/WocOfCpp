...
For the namespace it is used to encapsulate the functions that are used to
calculate the result of the expression. The main function is CommenCalculate,
which takes the input expression in the form of a vector For CommenCalculate, it
is the main function that will be called by the caller, and it will call the
other functions in the namespace to do the actual calculation. Most of the code
was code by ChatGPT actually
*/
#include <cmath>
#include <stack>
...
static bool isFunc(Token t) {
  return t == SIN || t == COS || t == TAN || t == SQRT || t == ABS ||
         t == LOG || t == LN || t == EXP;
}
...
static OpInfo info(const Op& op) {
  ...
  if (op.kind == OpKind::Tok &&
      (op.tok == MULT || op.tok == DIVI || op.tok == MOD))
    return {6, false, 2};

  if (op.kind == OpKind::Tok && (op.tok == PLUS || op.tok == MINU))
    return {5, false, 2};

  if (op.kind == OpKind::Tok && (op.tok == GREATER || op.tok == GREATER_EQUAL ||
                                 op.tok == LESS || op.tok == LESS_EQUAL))
    return {4, false, 2};
...
// and others as true as the number less than 1e-12 is considered as zero in the
// context of this calculator, it is reasonable to treat them as false
...
// pop2 pops two values from the stack and stores them in a and b (b is the top,
// a is the second top); returns false if the stack has less than 2 values
...
static bool apply(const Op& op, std::stack<double>& st, Error& err) {
  ...
  switch (op.tok) {
    ...
    case AND:
      st.push((isTruthy(a) && isTruthy(b)) ? 1.0 : 0.0);
      return true;
    case OR:
      st.push((isTruthy(a) || isTruthy(b)) ? 1.0 : 0.0);
      return true;
...
// operandLike returns true if the token can be an operand (number, variable or
// closing parenthesis), which is used to determine if a minus sign is unary or
// binary
...
std::string CommenCalculate(
  ...
  struct Rpn {
    ...
    std::string lex;
    Op op;
    static Rpn num(std::string s) {
      return {true, std::move(s), Op::fromTok(NUL)};
    }
    static Rpn oper(Op o) {
      Rpn r;
      r.isNum = false;
      r.op = o;
      return r;
  ...
  auto popByPrec = [&](const Op& cur) {
    ...
    while (!ops.empty()) {
      ...
      bool shouldPop = (!curi.rightAssoc && curi.prec <= topi.prec) ||
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    if (t == UNKNOWN || t == NUL) return GetErrorMessage(ERROR_SYNTAX_ERROR);

    if (t == VAR || t == FUN) return GetErrorMessage(ERROR_INVALID_ARGUMENT);
    ...
    if (t == RPARE) {
      ...
      if (!matched) return GetErrorMessage(ERROR_MISSING_PARENTHESIS);

      if (!ops.empty() && ops.top().kind == OpKind::Tok &&
          isFunc(ops.top().tok)) {
        out.push_back(Rpn::oper(ops.top()));
        ops.pop();
      }
  ...
  if (!sawAnyToken) return GetErrorMessage(ERROR_EMPTY_INPUT);
  ...
  for (const auto& it : out) {
    ...
    if (!apply(it.op, st, err)) return GetErrorMessage(err);
  }

  if (st.size() != 1) return GetErrorMessage(ERROR_SYNTAX_ERROR);
