...
The result of the function evaluation is then inserted back into the input
expression as a number token. Most of the code was code by ChatGPT actually
*/
#include "Calculators.h"

static bool containsFunToken(
    const std::vector<std::pair<Token, std::string>>& v) {
  for (auto& p : v)
    if (p.first == FUN) return true;
  return false;
}
